...
function List() {
    ...
    const getCompanies = async () => {
        try {
            const res = await HomeService.listCompanyAll();
    ...
    const getTypes = async (id) => {
        try {
            let res = await ListService.getListCompany(id);
            setIdTypes(id);
    ...
    return (infoType && idTypes && companies &&
        <>
            ...
            <div className="container mt-3 mb-5">
                <div className="scrollmenu">
                    {companies.map((type, index) => (
                        <a key={index} href={`#${type.description}`}
                           style={{
